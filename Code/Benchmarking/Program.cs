...
namespace Benchmarking
{
    class Program
    {
        ...
        {
            var res = BenchmarkRunner.Run<MyStringParserBenchmarks>();
        }
    }

    [MemoryDiagnoser]
    public class MyStringParserBenchmarks
    {
        private string testString = "this is,my,last thing";
        private MyStringParser Parser = new MyStringParser();

        [Benchmark(Baseline = true)]
        public void GetTheLastThing() => Parser.GetLastPart(testString);

        [Benchmark]
        public void GetTheLastThingFasterNot() => Parser.GetLastPartFaster(testString);

        [Benchmark]
        public void GetTheLastThingFasterStill() => Parser.GetLastPartWithFasterSplit(testString);
    }

    public class MyStringParser
    {
        private StringSplitter Splitter = new StringSplitter(1000);

        public string GetLastPart(string toBeParsed)
        {
            var parts = toBeParsed.Split(",");
            var lastPart = parts.LastOrDefault();
            return lastPart ?? string.Empty;
        }

        public string GetLastPartFaster(string toBeParsed)
        {
            var parts = toBeParsed.Split(",");
            return parts.LastOrDefault() ?? string.Empty;
        }

        public string GetLastPartWithFasterSplit(string toBeParsed)
        {
            var index = Splitter.Split(toBeParsed, ",");
            var lastPart = Splitter.buffer[index];
            return lastPart;
        }
    }


    /// Found at https://github.com/meikeric/String.Split
    /// <summary>
